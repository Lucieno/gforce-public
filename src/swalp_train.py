...
from torch_utils import argparser_distributed, nmod, adjust_learning_rate, load_swalp_state_dict
...
def swalp_quantize(param, bits=8, mode="stochastic"):
    ...
    temp = stochastic_rounding(i)
    return temp

def stochastic_rounding(data, bits=8, mode="nearest"):
    if mode == "stochastic":
        add_r_(data)
        data.floor_()
    elif mode == "nearest":
        data.round_()
    data.clamp_(-2 ** (bits - 1), 2 ** (bits - 1) - 1)
    return data
...
def quantize_bias_op(data, layer_op_name):
    x_exp, x_bits = store_configs[layer_op_name + "X"]
    y_exp, y_bits = store_configs[layer_op_name + "Y"]
    temp = data * 2 ** (-(x_exp - (x_bits - 2) + y_exp - (y_bits - 2)))
    temp = stochastic_rounding(temp, bits=16)
    return temp

def pre_quantize(param_x, param_y, param_z, layer_op_name, iter_time):
    ...
    z_q = quantize_bias_op(param_z, layer_op_name)
    return x_q, y_q, z_q

def pre_quantize_backward(param_x, param_y, layer_op_name, iter_time):
    named_x = NamedParam(layer_op_name + "X", param_x)
    named_y = NamedParam(layer_op_name + "Y", param_y)
...
class QuantizeMatMulFunction(Function):
    ...
    def forward(ctx, input, weight, bias, layer_name=None, iter_time=0):

        layer_op_name = layer_name + "Forward"
        InputQ, WeightQ, BiasQ = pre_quantize(input, weight, bias, layer_op_name, iter_time)

        OutputQ = InputQ.mm(WeightQ.t())
        OutputQ = OutputQ + BiasQ
    ...
    def backward(ctx, grad_output, ):
        ...
        if ctx.needs_input_grad[0]:
            ...
            OutputGradQ, WeightQ = pre_quantize_backward(grad_output, weight, layer_op_name, iter_time)
        ...
        if ctx.needs_input_grad[1]:
            ...
            OutputGradQ, InputQ = pre_quantize_backward(grad_output, input, layer_op_name, iter_time)
...
class QuantizeMatmul(nn.Module):
    ...
    def extra_repr(self):
        return 'in_features={}, out_features={}, bias={}, layer_name={}, iter_time={}'.format(
            self.input_features, self.output_features, self.bias is not None, self.layer_name, self.iter_time
        )
...
class QuantizeConv2dFunction(Function):
    @staticmethod
    # bias is an optional argument
    def forward(ctx, input, weight, bias, stride=1, padding=1, layer_name=None, iter_time=0):
        ...
        OutputQ = F.conv2d(InputQ, WeightQ, BiasQ, stride=stride, padding=padding)
...
class QuantizeConv2d(nn.Module):
    def __init__(self, NumInputChannel, NumOutputChannel, FilterHW, padding=1, stride=1, bias=True, layer_name=None):
        super().__init__()
...
if __name__ == "__main__":
    ...
    if test_to_run in ["vgg16_cifar10", "minionn_maxpool", "minionn_cifar10"]:
        ...
        if test_to_run == "vgg16_cifar10":
            main(test_to_run, NetQ_vgg16(10), trainset, testset)
        elif test_to_run == "minionn_maxpool":
            main(test_to_run, NetQ_MiniONN_maxpool(), trainset, testset)
        elif test_to_run == "minionn_cifar10":
            main(test_to_run, NetQ_MiniONN(), trainset, testset)
    if test_to_run in ["vgg"]:
